...
(() => {
  ...
  function onDocEvent(event) {
    ...
    else if (detail.t === "bye") removeReader(detail.clientId);
  ...
  function showChip(aid, rows, target) {
    ...
    if (existing !== undefined
      && (existing.button !== target.button || existing.chip.parentNode !== target.controls)) {
      dropChip(aid);
    }
    try {
      ...
      if (record === undefined) {
        ...
        target.controls.insertBefore(chip, target.button);

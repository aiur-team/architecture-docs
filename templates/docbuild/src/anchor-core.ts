...
const BLOCK_SET = new Set<string>(BLOCK);
...
function coreErr(offset: number, message: string): never {
  throw new Error(`anchor scan at ${offset}: ${message}`);
}

const isAsciiLetter = (c: string | undefined): boolean =>
  c !== undefined && c.length === 1 && /[A-Za-z]/.test(c);

const isAsciiDigit = (c: string | undefined): boolean =>
  c !== undefined && c.length === 1 && /[0-9]/.test(c);

const isAsciiAlnum = (c: string | undefined): boolean =>
  isAsciiLetter(c) || isAsciiDigit(c);

const isHexDigit = (c: string | undefined): boolean =>
  c !== undefined && c.length === 1 && /[0-9a-fA-F]/.test(c);

const isAsciiWhitespace = (c: string | undefined): boolean =>
  c !== undefined && c.length === 1 && /[ \t\n\r\f\v]/.test(c);
...
export function scanBlocks(fragment: string): ScannedBlock[] {
  ...
  const appendText = (s: string): void => {
    if (stack.length > 0) top().chunks.push(s);
  };

  // ------------------------------------------------------------------- entities

  const decodeReference = (amp: number): number => {
    ...
    if (next === "#") {
      ...
      const digitsStart = j;
      while (j < n && (hex ? isHexDigit(fragment[j]) : isAsciiDigit(fragment[j]))) j++;
      if (j === digitsStart) coreErr(amp, "numeric character reference is malformed");
      const after = fragment[j];
      if (isAsciiLetter(after)) coreErr(amp, "numeric character reference is malformed");
      if (after !== ";") coreErr(amp, 'numeric character reference is missing ";"');
      const value = parseInt(fragment.slice(digitsStart, j), hex ? 16 : 10);
      const scalar =
        value >= 1 && value <= 0x10ffff && !(value >= 0xd800 && value <= 0xdfff);
      if (!scalar) coreErr(amp, "numeric character reference is not a Unicode scalar value");
      appendText(String.fromCodePoint(value));
      return j + 1;
    }
    if (isAsciiLetter(next)) {
      let j = amp + 1;
      while (j < n && isAsciiAlnum(fragment[j])) j++;
      if (j >= n || fragment[j] !== ";") {
        coreErr(amp, 'named character reference is missing ";"');
      }
      const name = fragment.slice(amp + 1, j);
      const decoded = NAMED[name];
      if (decoded === undefined) coreErr(amp, `unknown named character reference "${name}"`);
      appendText(decoded);
      return j + 1;
    }
  ...
  const skipRawText = (open: number, openEnd: number, tag: string): number => {
    let m = openEnd;
    while (m < n) {
      const lt = fragment.indexOf("<", m);
      if (lt === -1) coreErr(open, `unclosed raw-text element ${tag}`);
      if (fragment[lt + 1] === "/") {
        let q = lt + 2;
        const ns = q;
        while (q < n && isTagNameChar(fragment[q])) q++;
        if (fragment.slice(ns, q).toLowerCase() === tag) {
          let r = q;
          while (r < n && isAsciiWhitespace(fragment[r])) r++;
          if (fragment[r] === ">") return r + 1;
        }
      }
      m = lt + 1;
    }
   ...
   */
  const readStartTag = (start: number): { name: string; end: number; selfClosing: boolean } => {
    let k = start + 1;
    const ns = k;
    while (k < n && isTagNameChar(fragment[k])) k++;
   ...
   */
  const skipOrdinaryToken = (start: number): number => {
    let k = start + 1;
    let quote: string | null = null;
    for (; k < n; k++) {
      const c = fragment[k]!;
  ...
  while (i < n) {
    ...
    if (c === "<") {
      if (fragment.startsWith("<!--", i)) {
        ...
        if (end === -1) coreErr(i, "unterminated HTML comment");
        i = end + 3;
        continue;
      }
      ...
      if (isAsciiLetter(fragment[nameStart])) {
        ...
        if (BLOCK_SET.has(tag.name)) {
          if (tag.selfClosing) coreErr(i, `self-closing block ${tag.name}`);
          stack.push({
    ...
    const runStart = i;
    while (i < n && fragment[i] !== "<" && !(fragment[i] === "&" && stack.length > 0)) i++;
    if (i > runStart) appendText(fragment.slice(runStart, i));
  }

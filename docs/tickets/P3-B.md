...
JSON strings must contain no lone surrogate or C0/C1 control character unless a field explicitly overrides that rule. The server-derived `actor.name` override below is required to preserve P2-G's complete valid identity domain. Unless a field says otherwise, strings are not trimmed or case-folded: the supplied value must already meet its grammar. Byte limits are UTF-8 byte limits, not JavaScript code-unit limits; the actor-name limit is the one explicit code-unit exception.
...
| `actor` | Exact keys `sub`, `name`, `email`. `sub` passes P2-G `assertIdentitySub()`. `name` is any string of at most 200 JavaScript UTF-16 code units, including empty, multibyte, C0/C1, or lone-surrogate values; this field explicitly overrides the general string rule and is never repaired. `email` is either `""` or unchanged by P2-G `normalizeEmail()` and therefore at most 254 ASCII/UTF-8 bytes. |
